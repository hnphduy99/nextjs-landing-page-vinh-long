...
export async function POST(request: Request) {
  ...
  try {
    ...
    await logActivity(type as string, 'thêm', (data as any).title || (data as any).name || 'Nội dung');
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    await logActivity(type as string, 'sửa', (data as any).title || (data as any).name || 'Nội dung');
...
export async function DELETE(request: Request) {
  ...
  try {
    let data;
    if (type === 'milestones') data = await prisma.historicalMilestone.delete({ where: { id } });
    else if (type === 'personalities') data = await prisma.notablePersonality.delete({ where: { id } });
    else if (type === 'features') data = await prisma.culturalFeature.delete({ where: { id } });
    else if (type === 'festivals') data = await prisma.festival.delete({ where: { id } });
    else return NextResponse.json({ error: 'Invalid type' }, { status: 400 });

    await logActivity(type as string, 'xóa', (data as any).title || (data as any).name || 'Nội dung');
